	...
	const renderCommentItem = ({ item }) => {
		return (
			<View style={{ margin: 10 }}>
				...
				<Rating imageSize={10} readonly startingValue={item.rating} style={styles.ratingItem} />
			...
			<ScrollView>
				...
				<Modal animationType={"slide"} transparent={false} visible={this.state.showModal} onRequestClose={() => this.toggleModal()}>
					<View style={styles.modal}>
						<Rating showRating imageSize={40} startingValue={this.state.rating} style={{ paddingVertical: 10 }} onFinishRating={(rating) => this.setState({ rating: rating })} />

						<Input placeholder="Author" leftIcon={{ type: "font-awesome", name: "user-o" }} leftIconContainerStyle={styles.inputsIcon} onChangeText={(value) => this.setState({ author: value })} value={this.state.author} />
						<Input
							placeholder="Comment"
							leftIcon={{
								type: "font-awesome",
						...
						/>
						<Button title="SUBMIT" buttonStyle={styles.submitBtn} onPress={() => this.handleComment(campsiteId)} />
